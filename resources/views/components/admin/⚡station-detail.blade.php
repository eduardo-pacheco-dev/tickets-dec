...
<div class="space-y-6">
    ...
    <div class="overflow-hidden rounded-2xl border border-zinc-200 bg-gradient-to-br from-white to-zinc-50 dark:border-zinc-700 dark:from-zinc-800 dark:to-zinc-800/60">
        <div class="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between lg:p-8">
            <div class="flex items-start gap-4">
                <div class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-zinc-900 text-white shadow-sm dark:bg-white dark:text-zinc-900">
                ...
                <div>
                    ...
                    <flux:text class="mt-1">
                        {{ $this->value($this->station->street) }}
                        {{ $this->station->city ? $this->value($this->station->city) : '' }}{{ $this->station->state ? '/'.$this->station->state : '' }}
                    </flux:text>
                    <div class="mt-3 flex flex-wrap gap-2">
                        <flux:badge color="zinc" size="sm" inset="top bottom">
                            <flux:icon name="signal" class="size-3.5" />
                            {{ $this->value($this->station->technology) }}
                        </flux:badge>
                        <flux:badge color="zinc" size="sm" inset="top bottom">
                            <flux:icon name="building-office-2" class="size-3.5" />
                            {{ $this->value($this->station->element_type) }}
                        </flux:badge>
                        @if ($this->station->classification)
                            <flux:badge color="zinc" size="sm" inset="top bottom">
                                <flux:icon name="tag" class="size-3.5" />
                                {{ $this->station->classification }}
                            </flux:badge>
                        @endif
                        @if ($this->station->status)
                            <flux:badge color="blue" size="sm" inset="top bottom">{{ $this->station->status }}</flux:badge>
                        @endif
                    </div>
                </div>
            </div>
